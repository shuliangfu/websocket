...
WebSocket utility library for real-time communication apps, push services,
...
For multiple WebSocket server instances, use a distributed adapter for message
sync. Adapters handle:
...
By default, the server uses the in-memory adapter (single-server). For
multi-server, configure Redis or MongoDB adapter.
...
- `messageCache?: { maxSize?, ttl? } | false`: Message cache config (default:
  enabled)
- `messageQueue?: { maxSize?, batchSize?, processInterval? } | false`: Message
  queue config (default: enabled)
...
- `on(event: "connection", listener: ServerEventListener): void`: Connection
  listener
...
MongoDB distributed adapter, Change Streams or polling for multi-server
communication.
...
- **MessageQueue**: `useMessageQueue=true` for broadcast/emitToRoom via queue,
  backpressure
- **BatchHeartbeatManager**: `useBatchHeartbeat=true` for centralized heartbeat,
  fewer timers
...
See [OPTIMIZATION.md](../../OPTIMIZATION.md) and
[OPTIMIZATION_ANALYSIS.md](../../OPTIMIZATION_ANALYSIS.md) for details.

---

## 🌐 Client Support

See [client/README.md](./client/README.md) for WebSocket client usage.
